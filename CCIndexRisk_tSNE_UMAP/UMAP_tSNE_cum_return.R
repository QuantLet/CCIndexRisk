...
if (file.exists("Data/portfolio_cum_return.qs")){
  data_cum <- qread("Data/portfolio_cum_return.qs")
}else{
...
### Calculate tSNE
set.seed(12345)
tsne_data <- Rtsne(t(data_cum), dims=2)
tsne_plot <- data.frame(x = tsne_data$Y[,1], y = tsne_data$Y[,2])

### Divide data by optimalization method
p_type <- rep(NA, nrow(tsne_plot))
p_type[str_detect(colnames(data_cum), "ES")] <- "ES"
p_type[str_detect(colnames(data_cum), "VAR")] <- "VaR"
p_type[str_detect(colnames(data_cum), "PWR")] <- "PWR"
p_type[str_detect(colnames(data_cum), "EXP")] <- "EXP"
p_type[str_detect(colnames(data_cum), "Index")] <- "Index"
p_type[str_detect(colnames(data_cum), "Index_5")] <- "CRIX"
...
p_reb[str_detect(colnames(data_cum), "_30")] <- "30"
p_reb[str_detect(colnames(data_cum), "_90")] <- "90"
p_reb[str_detect(colnames(data_cum), "Index")] <- "Index"
p_reb[str_detect(colnames(data_cum), "Index_5")] <- "CRIX"
p_reb <- factor(as.factor(p_reb), levels = c("30", "90", "Index", "CRIX"))

### Divide data by maximum weight
p_max <- rep(NA, nrow(tsne_plot))
p_max[str_detect(colnames(data_cum), c("ES_|VAR_|PWR_|EXP_"))] <- "1"
p_max[str_detect(colnames(data_cum), c("ES20_|VAR20_|PWR20_|EXP20_"))] <- "0.2"
p_max[str_detect(colnames(data_cum), c("ES30_|VAR30_|PWR30_|EXP30_"))] <- "0.3"
p_max[str_detect(colnames(data_cum), c("ES50_|VAR50_|PWR50_|EXP50_"))] <- "0.5"
p_max[str_detect(colnames(data_cum), "Index")] <- "Index"
p_max[str_detect(colnames(data_cum), "Index_5")] <- "CRIX"
p_max <- factor(as.factor(p_max), levels = c("0.2", "0.3", "0.5", "1", "Index", "CRIX"))

### Outlier indices numbers 
ind <- c(rep(NA, 2208), rep(NA, 12), "13", rep(NA, 5), "19", NA, NA, "22")
...
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/tSNE/cummulative/p_type.png", gg, width = 5000, height = 4500, 
...
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/tSNE/cummulative/p_reb.png", gg, width = 5000, height = 4500, 
       dpi = 500, units = "px", limitsize = F)
...
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/tSNE/cummulative/p_max.png", gg, width = 5000, height = 4500, 
       dpi = 500, units = "px", limitsize = F)
...
gg <- ggplot2::ggplot(umap_plot, aes(x=x, y=y, col=p_type, order = as.numeric(factor(p_type))))+ 
  ...
  scale_shape_manual(values=c(17, 17, 19, 19, 15, 15))
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/UMAP/cummulative/p_type.png", gg, width = 5000, height = 4500, 
       dpi = 500, units = "px", limitsize = F)
...
gg <- ggplot2::ggplot(umap_plot, aes(x=x, y=y, col=p_reb, order = as.numeric(factor(p_reb))))+ 
  ...
  scale_shape_manual(values=c(17, 19, 15, 15))
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/UMAP/cummulative/p_reb.png", gg, width = 5000, height = 4500, 
       dpi = 500, units = "px", limitsize = F)
...
gg <- ggplot2::ggplot(umap_plot, aes(x=x, y=y, col=p_max, order = as.numeric(factor(p_max))))+ 
  ...
  scale_shape_manual(values=c(17, 17, 17, 19, 15, 15))
gg
ggsave("CCIndexRisk_tSNE_UMAP/Output/UMAP/cummulative/p_max.png", gg, width = 5000, height = 4500, 
       dpi = 500, units = "px", limitsize = F)
